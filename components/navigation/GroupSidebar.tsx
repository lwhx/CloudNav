      ...
      {groups.map(group => {
        ...
        return (
          <div key={group.id} className="space-y-1">
            <button onClick={() => onSelect(group.id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition-all ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-slate-600 hover:bg-white hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white'}`} aria-current={active ? 'page' : undefined}>
              ...
              <Icon name={active ? 'ChevronDown' : 'ChevronRight'} size={15} className={active ? 'text-blue-100' : 'text-slate-300'} />
            </button>
            {active && group.categories.length > 0 && (
